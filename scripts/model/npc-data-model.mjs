...
export class NpcDataModel extends foundry.abstract.DataModel {
  static defineSchema() {
    const { StringField, NumberField, EmbeddedDataField, SchemaField, BooleanField, TypedObjectField } =
      foundry.data.fields;
    return {
    ...
    return {
      ...
      system: {
        ...
        attributes: Object.fromEntries(
          Object.entries(this.attributes).map(([key]) => [key, { base: this.attributeValue(key) }]),
        ),
        bonuses: {
          accuracy: {
        ...
        immunities: Object.fromEntries(
          Object.entries(this.statusImmunities).map(([key, value]) => [key, { base: value }]),
        ),
        resources: {
          hp: {
        ...
        affinities: Object.fromEntries(
          Object.entries(this.affinities).map(([key, value]) => [key, { base: systemAffinityValues[value.value] }]),
        ),
        isElite: { value: this.rank === 'elite' },
        isChampion: {
          value: this.rank.startsWith('champion') ? Number(this.rank.substring(8)) : 1,
        },
        rank: {
