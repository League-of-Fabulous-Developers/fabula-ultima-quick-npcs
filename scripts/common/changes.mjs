...
function applySimple(model, jsonModel, choices) {
    Object.entries(jsonModel).forEach(([key, value]) => {
        if (Array.isArray(value)) {
            const dupe = foundry.utils.duplicate(value).map(element => typeof element === "string" ? game.i18n.format(element, choices) : element);
...
const stepMapping = {
    AssignSpeciesVulnerability: (context, step) => AssignSpeciesVulnerabilityStep.addSpeciesVulnerability(context, step.options),
    ...
    ConditionalBonusSkill: (context, step, spellLists) => ConditionalBonusSkillStep.addConditionalBonusSkill(context, {
        drawback: parseSkill(step.drawback, spellLists),
        options: Object.fromEntries(Object.entries(step.options).map(([key, value]) => [key, parseSkill(value, spellLists)])),
    }),
    ChooseCustomization: (context, step, spellLists) => ChooseCustomizationStep.addCustomization(context, Object.fromEntries(Object.entries(step.options).map(([key, value]) => [key, parseSkill(value, spellLists)]))),
    ConfigureAttack: (context, step, spellLists) => {
        ...
        if (step.configure.special) {
            ConfigureAttackStep.configureSpecialChoice(context, attack, Object.fromEntries(Object.entries(step.configure.special).map(([key, value]) => [key, parseSkill(value, spellLists)])))
        }
    },
    ChooseSpell: (context, step, spellLists) => {
        let spells;
        if (typeof step.spells === "string") {
            spells = Object.fromEntries(Object.entries(spellLists[step.spells]).map(([key, value]) => [key, parseSpell(value, key)]));
        } else {
            ...
            if (step.type === "ChooseSpell") {
                if (typeof step.spells === "string") {
                    if (!spellLists[step.spells]) {
                        ui.notifications.error(game.i18n.format("QUICKNPC.error.unknownSpellList", {spellList: step.spells}))
...
export function parseChanges(data, spellLists = {}, vulnAsSpeciesVuln = false, skillChoices = {}) {
    ...
    if (data.conditional) {
        for (const conditionalChange of data.conditional) {
            if (!(conditionalChange.choice in skillChoices)) {
                ui.notifications.error(game.i18n.format("QUICKNPC.error.unknownChoice", {choice: conditionalChange.choice}))
                ...
                for (const value of conditionalChange.values) {
                    if (!(value in skillChoices[conditionalChange.choice].options)) {
                        ui.notifications.error(game.i18n.format("QUICKNPC.error.unknownChoiceOption", {
    ...
    return (model, context, choices = {}) => {
        choices = Object.fromEntries(Object.entries(choices).map(([key, value]) => [key, game.i18n.localize(skillChoices[key]?.options[value] ?? value)]));
        if (data.model) {
            applyToModel(model, data.model, choices, vulnAsSpeciesVuln)
        }
        if (data.steps) {
