  ...
  static getTemplateData(formValues, current, context) {
    ...
    if (attack[attributeKey]) {
      attributes = Object.fromEntries(
        attack[attributeKey].map(([attr1, attr2]) => [
          `${attr1}+${attr2}`,
    ...
    if (attack[damageTypeKey]) {
      damageTypes = Object.fromEntries(
        attack[damageTypeKey].map((damageType) => [damageType, CONSTANTS.damageTypes[damageType]]),
      );
    }
    ...
    if (special && formValues.special && special[formValues.special].choices) {
      nestedChoices = Object.fromEntries(
        Object.entries(special[formValues.special].choices ?? {}).filter(([, value]) => {
          return (
            !value.conditional ||
            Object.entries(value.conditional).every(([key, value]) => formValues[`special.${key}`] === value)
          );
        }),
      );
      selectedNested = Object.fromEntries(
        Object.keys(nestedChoices).map((key) => {
  ...
  #validateSpecial(specialOptions, special, nested) {
    ...
    return Object.entries(specialOptions[special].choices ?? {})
      .filter(([, value]) => {
        return (
          !value.conditional ||
          Object.entries(value.conditional).every(([condition, value]) => nested[condition] === value)
        );
      })
